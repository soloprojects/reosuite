...
class InventoryContract extends Model
{
    ...
    public function statusType(){
        return $this->belongsTo('App\model\InventoryContractStatus','contract_status','id')->withDefault();

    }

    public function contract(){
        return $this->belongsTo('App\model\InventoryContractType','contract_type','id')->withDefault();
    ...
    public function bomData(){
        return $this->hasMany('App\model\InventoryContractItems','contract_id','id');
    ...
    public static function searchContract($value){
        return static::join('inventory_contract_type', 'inventory_contract_type.id', '=', 'inventory_contract.contract_type')
            ->join('inventory_contract_status', 'inventory_contract_status.id', '=', 'inventory_contract.contract_status')
            ->join('vendor_customer', 'vendor_customer.id', '=', 'inventory_contract.customer_id')
            ->where('inventory_contract.status', '=',Utility::STATUS_ACTIVE)
            ->where(function ($query) use($value){
                $query->where('inventory_contract.name','LIKE','%'.$value.'%')
                    ->orWhere('inventory_contract_status.name','LIKE','%'.$value.'%')
                    ->orWhere('inventory_contract_type.name','LIKE','%'.$value.'%')
                    ->orWhere('inventory_contract.invoice_id','LIKE','%'.$value.'%')
